...
static int airoha_tx_meter_get_rate(struct airoha_gdm_port *port, int q)
{
	u32 rate, unit, tick, val, trtcm_cfg, rl_mode_cfg;
	int err;

	err = airoha_qdma_get_rate_limit_param(port->qdma, q,
					       REG_EGRESS_QUEUE_TRTCM_CFG,
					       TRTCM_MISC_MODE,
					       &rl_mode_cfg, NULL);
	if (err)
...
static int airoha_xmit_rings_show(struct seq_file *s, void *data)
{
	...
	seq_puts(s, "     queue | hw-queued |      head |      tail |      rate |\n");
	for (i = 0; i < ARRAY_SIZE(qdma->q_tx); i++) {
		...
		rate = airoha_tx_meter_get_rate(port, i);
		seq_printf(s, " %9d | %9d | %9d | %9d | %9d |\n",
			   i, q->queued, q->head % q->ndesc,
			   q->tail % q->ndesc, rate);
	}
