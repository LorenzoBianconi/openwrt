...
static int airoha_spi_write_one_byte_with_cmd(struct airoha_snand *as, unsigned char cmd, const u16 *data)
{
    ...
    airoha_spi_write_data_fifo(as, (unsigned char *)data, 1);
    return 0;
}
...
static int airoha_snand_op_transfer(struct airoha_snand *as, const struct spi_mem_op *op)
{
    ...
    for(idx = 0; idx < op->addr.nbytes; idx++)
    {
        ...
        {
            airoha_spi_write_one_byte_with_cmd(as, 0x11, (u16 *)&addr_data);
        }
        else
        {
            airoha_spi_write_one_byte_with_cmd(as, 0x8, (u16 *)&addr_data);
        }
    }
    ...
    for(idx = 0; idx < op->dummy.nbytes; idx++)
    {
        ...
        airoha_spi_write_one_byte_with_cmd(as, 0x8, (u16 *)&data);
    }
    ...
    {
        airoha_spi_write_nbytes_with_cmd(as, 0x8, (unsigned char *)op->data.buf.out, op->data.nbytes);
    }
